...
interface IProps {
  ...
  orderDeleteById: (arg: IOrderDeleteById) => void;
  queryParams: IOrderQueryParams;
}
...
const ActionMenu = (props: IProps) => {
  ...
  const deletePrompt = (order: IOrder) => {
    Modal.confirm({
      ...
      content: `${order.name}`,
      okType: 'danger',
      onOk: () => props.orderDeleteById({ orderId: order._id, queryParams }),
    });
  };
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  orderDeleteById: (payload: IOrderDeleteById) => dispatch({ type: 'OrderDashboard/orderDeleteById', payload }),
});
